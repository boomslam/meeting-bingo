...
## 🐳 CONTAINER EXECUTION (Docker Skill)

**All package installations and code execution happen inside Docker containers, NOT on the host machine.**
...
**Container name for this project: `meeting-bingo-dev-1`**
...
docker exec -it meeting-bingo-dev-1 npm run build
```
...
```bash
docker ps --filter "name=meeting-bingo-dev-1" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
```
...
```bash
docker exec meeting-bingo-dev-1 npm install <package>     # Install package
docker exec meeting-bingo-dev-1 npm test                  # Run tests
docker exec meeting-bingo-dev-1 npm run build             # Build
